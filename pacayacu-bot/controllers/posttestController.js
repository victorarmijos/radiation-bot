...
async function iniciarPosttest(telefono, idioma = 'es') {
  ...
  setTimeout(async () => {
    await enviarBotones(telefono, PREGUNTAS_POSTTEST[0].texto_es, [
      { id: 'A', title: 'Opción A' },
      { id: 'B', title: 'Opción B' },
      { id: 'C', title: 'Opción C' }
    ]);
...
async function procesarRespuestaPosttest(telefono, respuesta, etapaActual, idioma = 'es') {
  ...
  if (!['A', 'B', 'C'].includes(cmd)) {
    await enviarBotones(telefono, '⚠️ Responde con *A*, *B* o *C*', [
      { id: 'A', title: 'Opción A' },
      { id: 'B', title: 'Opción B' },
      { id: 'C', title: 'Opción C' }
    ]);
  ...
  if (siguienteIndex < PREGUNTAS_POSTTEST.length) {
    ...
    setTimeout(async () => {
      await enviarBotones(telefono, PREGUNTAS_POSTTEST[siguienteIndex].texto_es, [
        { id: 'A', title: 'Opción A' },
        { id: 'B', title: 'Opción B' },
        { id: 'C', title: 'Opción C' }
      ]);
